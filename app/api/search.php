...
class search extends AWS_CONTROLLER
{
    ...
    public function index_action()
    {
        ...
        if ($result) {
            $key_arr = array('type', 'search_id', 'name', 'detail', 'url');

            foreach ($result as $key => $val) {
                if ($val['type'] == 'users') {
                    if ($this->model('follow')->user_follow_check($this->user_id, $val['uid'])) {
                        $result[$key]['detail']['has_focus'] = 1;
                    } else {
                        $result[$key]['detail']['has_focus'] = 0;
                    }
                }
